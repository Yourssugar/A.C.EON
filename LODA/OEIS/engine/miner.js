...
function matchDepth(prog, target, runOptions){
  ...
  for(let d = 0; d < target.length; d++){
    ...
    if(v !== target[d]) return d;
  }
  return target.length; // полное совпадение
}

function mine(target, options){
  const opts = Object.assign({}, DEFAULTS, options || {});
  const runOptions = opts.runOptions;
  const seq = target.map(x => (typeof x === 'bigint') ? x : BigInt(x));
  ...
  function consider(prog){
    ...
    seen.add(text);
    attempts++;
    const depth = matchDepth(prog, seq, runOptions);
    if(depth < 0) return;
