...
function skyNow(){ const now=new Date(); const m=moonNow(now);
  return { moonSign:signFromLon(eclLon('moon',now)), moonPhaseName:m.name, moonIllum:m.illum, mercuryRetro:isRetro('mercury',now) }; }
...
function buildToday(){
  ...
  const sunLon=eclLon('sun',new Date(Date.UTC(y||2000,m-1,d,12)));
  const sunSign=signFromLon(sunLon);
  const card=dailyForecast(sunSign, skyNow(), new Date());
  renderForecast(card);
  save({tab:'today',today:{y,m,d}});
}
function renderForecast(c){
  $('#chart').classList.add('hidden'); $('#hint').style.display='none';
...
function showTab(name){
  ...
  // сцена: «Сегодня» → карточка, остальное → колесо
  if(name==='today'){ $('#chart').classList.add('hidden'); }
  else { $('#forecast-card').classList.add('hidden'); }
  closePanel();
}
function initCities(){
...
function particles(){ const cv=$('#stars'),cx=cv.getContext('2d');let W,H,dots;
  const rz=()=>{W=cv.width=innerWidth;H=cv.height=innerHeight;dots=Array.from({length:80},()=>({x:Math.random()*W,y:Math.random()*H,r:Math.random()*1.3+.2,s:Math.random()*.14+.02,a:Math.random()*.5+.2}));};
  rz();addEventListener('resize',rz);
  (function loop(){cx.clearRect(0,0,W,H);dots.forEach(d=>{d.y-=d.s;if(d.y<0){d.y=H;d.x=Math.random()*W;}cx.beginPath();cx.arc(d.x,d.y,d.r,0,7);cx.fillStyle=`rgba(201,162,75,${d.a})`;cx.fill();});requestAnimationFrame(loop);})();
}
