...
// CNH (натал-движок) грузится лениво — берём его через window.CNH в момент использования
...
function fullChart(y, m, d, hh, mm, lat, lon) {
  const CNH = window.CNH;
  if (!CNH) throw new Error('natal.bundle.js ещё не загружен');
...
function ensureNatalEngine() {
  ...
  _cnhLoading = new Promise((res, rej) => {
    ...
    s.src = 'natal.bundle.js';   // плоская раскладка: лежит рядом с index.html
