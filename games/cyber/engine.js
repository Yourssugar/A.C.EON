...
function goHome(){if(typeof window!=='undefined'&&window.CYBER_HOME){window.CYBER_HOME();}else{location.href='../../index.html';}}
...
function renderDiffPicker(){
  ...
  wrap.appendChild(grid);root.appendChild(wrap);const _bk=root.querySelector('.ce-back');if(_bk)_bk.onclick=e=>{e.preventDefault();goHome();};}
...
function renderShell(){
  ...
  $('#ce-reset').onclick=()=>{store.set('bresh:diff:'+E.course.id,null);E.diff=null;boot(E.course);};const _bk=root.querySelector('.ce-back');if(_bk)_bk.onclick=e=>{e.preventDefault();goHome();};
  renderLadder();}
...
function solve(lv,delay){
  ...
  else{const n=el('button','ce-primary','вернуться на портал');n.onclick=goHome;act.appendChild(n);}
